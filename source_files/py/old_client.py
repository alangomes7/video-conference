...
def client_receive():
    ...
    while not stop_client:
        try:
            ...
            thread = threading.Thread(target=process_messages, args=(message_buffer.get_messages()), daemon=True)
            thread.start()
...
def process_messages(message_buffer_messages):
    """
    Process buffered messages.
    """
    for i in message_buffer_messages:
        message = message_buffer_messages[i]
